module.exports = (container) => {
  ...
  const { questionRepo } = container.resolve('repo')
  ...
  const getListQuestion = async (req, res) => {
    try {
      ...
      Object.keys(search).forEach(key => {
        ...
        const pathType = (Question.schema.path(key) || {}).instance || ''
      ...
      const data = await questionRepo.getListQuestion(pipe, perPage, skip, sort)
      const total = await questionRepo.getCount(pipe)
  ...
  const getQuestionById = async (req, res) => {
    try {
      ...
      const news = await questionRepo.getQuestion(query)
      if (news) {
  ...
  const updateQuestionById = async (req, res) => {
    try {
      ...
      const body = req.body
      if (id && id.length === 24) {
        const item = await questionRepo.getQuestionById(id)
        if (item) {
          ...
          const {
            error,
            value
          } = await schemaValidator(body, 'Question')
          if (error) {
            logger.e(error)
          ...
          await questionRepo.updateQuestion(id, value)
          return res.status(httpCode.SUCCESS).json({ ok: true })
        }
      }
      return res.status(httpCode.BAD_REQUEST).json({ msg: 'BAD REQUEST' })
  ...
  const createQuestion = async (req, res) => {
    try {
      ...
      if (body) {
        ...
        const news = await questionRepo.createQuestion(value)
