module.exports = (container) => {
  ...
  const { partSubjectRepo } = container.resolve('repo')
  ...
  const getListPartSubject = async (req, res) => {
    try {
      let {
        page,
        perPage,
      ...
      Object.keys(search).forEach(key => {
        ...
        const pathType = (PartSubject.schema.path(key) || {}).instance || ''
      ...
      const data = await partSubjectRepo.getListPartSubject(pipe, perPage, skip, sort)
      const total = await partSubjectRepo.getCount(pipe)
  ...
  const getPartSubjectById = async (req, res) => {
    try {
      let { id } = req.params
      id = decodeURI(id)
      ...
      const news = await partSubjectRepo.getPartSubject(query)
      if (news) {
  ...
  const removePartSubjectById = async (req, res) => {
    try {
      const { id } = req.params
      if (id && id.length === 24) {
        await partSubjectRepo.deletePartSubject(id)
  ...
  const updatePartSubjectById = async (req, res) => {
    try {
      const { id } = req.params
      const body = req.body
      if (id && id.length === 24) {
        ...
        if (item) {
          ...
          const {
            error,
            value
          } = await schemaValidator(body, 'PartSubject')
          if (error) {
            logger.e(error)
          ...
          await partSubjectRepo.updatePartSubject(id, value)
          return res.status(httpCode.SUCCESS).json({ ok: true })
        }
      }
      return res.status(httpCode.BAD_REQUEST).json({ msg: 'BAD REQUEST' })
  ...
  const createPartSubject = async (req, res) => {
    try {
      ...
      if (body) {
        ...
        const news = await partSubjectRepo.createPartSubject(value)
  ...
  return {
    getListPartSubject,
    getPartSubjectById,
    removePartSubjectById,
    updatePartSubjectById,
    createPartSubject
  }
}
